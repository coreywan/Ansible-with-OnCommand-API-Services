...
def get():
    ...
    if is_enabled != None:
        if flag is 0:
            url_path+="?is_enabled="+is_enabled
            flag=1
        else:
            url_path+="&is_enabled="+is_enabled
    if comment != None:
    ...
    if duration != None:
        if flag is 0:
            url_path+="?duration="+duration
            flag=1
        else:
            url_path+="&duration="+duration
    if qos_policy != None:
        if flag is 0:
            url_path+="?qos_policy="+qos_policy
            flag=1
        else:
            url_path+="&qos_policy="+qos_policy
    if sortBy != None:
...
def post():
    ...
    if (is_enabled != None) & (is_enabled != key):
        payload['is_enabled']=is_enabled
    if (comment != None) & (comment != key):
        payload['comment']=comment
    if (duration != None) & (duration != key):
        payload['duration']=duration
    if (qos_policy != None) & (qos_policy != key):
...
def main():
        fields = {
                ...
                "is_enabled" : {"required": False, "type": "str"},
        ...
        global qos_policy
        qos_policy = module.params["qos_policy"]
        global sortBy
