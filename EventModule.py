...
def get():
    ...
    if type != None:
        if flag is 0:
            ...
            url_path+="&type="+type
    if source_resource_key != None:
        if flag is 0:
            url_path+="?source_resource_key="+source_resource_key
            flag=1
        else:
            url_path+="&source_resource_key="+source_resource_key
    if sortBy != None:
...
def post():
    ...
    if (key != None) & (key != key):
        payload['key']=key
    if (severity != None) & (severity != key):
        payload['severity']=severity
    if (created_timestamp != None) & (created_timestamp != key):
        payload['created_timestamp']=created_timestamp
    if (type != None) & (type != key):
        payload['type']=type
    if (source_resource_key != None) & (source_resource_key != key):
        payload['source_resource_key']=source_resource_key
...
def main():
        fields = {
                ...
                "source_resource_key" : {"required": False, "type": "str"},
        ...
        global source_resource_key
        source_resource_key = module.params["source_resource_key"]
        global sortBy
