...
def get():
    ...
    if igroup_key != None:
        if flag is 0:
            url_path+="?igroup_key="+igroup_key
            flag=1
        else:
            url_path+="&igroup_key="+igroup_key
    if lun_key != None:
    ...
    if host_lun_id != None:
        if flag is 0:
            url_path+="?host_lun_id="+host_lun_id
            flag=1
        else:
            url_path+="&host_lun_id="+host_lun_id
    if sortBy != None:
...
def post():
    ...
    if (igroup_key != None) & (igroup_key != key):
        payload['igroup_key']=igroup_key
    if (lun_key != None) & (lun_key != key):
        payload['lun_key']=lun_key
    if (host_lun_id != None) & (host_lun_id != key):
        payload['host_lun_id']=host_lun_id
...
def main():
        fields = {
                ...
                "host_lun_id" : {"required": False, "type": "str"},
        ...
        global igroup_key
        igroup_key = module.params["igroup_key"]
        global lun_key
        lun_key = module.params["lun_key"]
        global host_lun_id
        host_lun_id = module.params["host_lun_id"]
        global sortBy
