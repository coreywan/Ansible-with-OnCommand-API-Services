...
def get():
    ...
    if storage_vm_key != None:
        if flag is 0:
            ...
            url_path+="&storage_vm_key="+storage_vm_key
    if routing_group_name != None:
        if flag is 0:
            url_path+="?routing_group_name="+routing_group_name
            flag=1
        else:
            url_path+="&routing_group_name="+routing_group_name
...
def post():
    ...
    if (routing_group_name != None) & (routing_group_name != key):
...
def main():
        ...
        global routing_group_name
        routing_group_name = module.params["routing_group_name"]
